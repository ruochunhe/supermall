<template>
  <div>
    <div class="hy-swiper">
      ...
      <div class="indicator">
        <ul>
          <li v-for="(item,index) in banners" :key="item.acm" class="indicatorItem" :class="{active: currentIndex == index + 1 || currentIndex == index-0+5 || currentIndex == index - 3}" :index="index + 1"></li>
        </ul>
      </div>
    </div>
  </div>
  ...
  methods: {
    ...
    startTimer: function () {
     this.playTimer = setInterval(() => {
        //每隔1500ms动一次
        this.currentIndex++;
        this.swiperStyle.transition = "transform 500ms";
